...
##함수 선언 부분 ##
def loadImage (fname) :
    global window, canvas, paper, filename, XSIZE, YSIZE, inlmage, filename
...
def displayImage (image) :
    ...
    for i in range(0, XSIZE) :
        ...
        for k in range(0, YSIZE) :
            data = image[i][k]
            tmpString +="#%02x%02x%02x " % (data, data, data) # x 뒤에 한한 공백
        rgbString += "{" + tmpString + "} "
    paper.put(rgbString)

def func_open() :
    global window, canvas, paper, filename, XSIZE, YSIZE, inlmage
    ...
    window.geometry(str(XSIZE) + 'x' + str(YSIZE)) + 291
