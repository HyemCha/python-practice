...
def func_open():
    global window, canvas, paper, photo, photo2, oriX, oriY
    readFp = askopenfilename(parent=window, filetypes = (("모든 그림 파일", "*.jpg; *.jpeg; *.bmp; *.png; *.tif; *gif"),("모든 그림 파일", "*.*")))
...
def func_mirror(loc):
    global window, canvas, paper, photo, photo2, oriX, oriY
    photo2 = photo.copy()
    photo2 = photo2.transpose(loc)
    newX = photo2.width
    newY = photo2.height
    displayImage(photo2, newX, newY)

def func_mirror1():
    func_mirror(Image.Transpose.FLIP_TOP_BOTTOM)

def func_mirror2():
    func_mirror(Image.Transpose.FLIP_LEFT_RIGHT)
...
fileMenu.add_command(label="프로그램 종류", command=func_exit)
